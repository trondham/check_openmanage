...
the two blacklists from the config file and command line are added
together.
...
If used together with the command line option C<-t> or C<--timeout>,
...
The corresponding command line option is C<-p> or <--perfdata>.
...
The corresponding command line option is C<--tempunit>, which will
override the configuration option if used. The command line option
C<-F> or C<--fahrenheit> will also override this option.
...
as the command line options. Example:
...
options, if used simultaneously.

=back

=head2 SNMP OPTIONS

Several SNMP related options may be set in the configuration file. For
each of these, if used together with the corresponding command line
option, the latter takes presedence and overrules the option set in
the configuration file. The configuration file may contain the
following SNMP options:
...
These options all corresponds to command line options. If the
configuration file option and the corresponding command line option is
used simultaneously, the command line option takes precedence.
