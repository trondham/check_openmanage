...
sub check_memory {
  ...
  DIMM:
	...
	if ($snmp) {
	    ...
	    $size     = defined $out->{memoryDeviceExtendedSize}
	      ? (sprintf '%d MB', ($out->{memoryDeviceExtendedSize} || 0))
		: (sprintf '%d MB', ($out->{memoryDeviceSize} || 0)/1024);
  ...
  AMP:
	...
	if ($snmp) {
	    ...
	    $reading  = $type eq 'amperageProbeTypeIsDiscrete'
	      ? (get_hashval($out->{amperageProbeDiscreteReading}, \%amp_discrete) || 'Unknown reading')
		: ($out->{amperageProbeReading} || 0);
	    my $foo = get_hashval($out->{amperageProbeDiscreteReading}, \%amp_discrete);
